/*
    Example of requesting multi capture triggered time data.

    This example shows you how to use the node to complete
    mulitple captures of time data. Capture is performed once
    the given power threshold is reached. You can specify to
    capture X number of these triggers.
...
int main(int argc, char* argv[])
{
	...
	T_PACKET* txpacket, *rxpacket;
	int seg = 0;
    int nseg = 0;
	//ncp_client = connect_to_node("217.37.1.115");
	ncp_client = connect_to_node("192.168.1.99");
    /* Request a power threshold multiple time capture
     *
     * This request does 10 X captures of 4096 samples
     * at 900Mhz each time the power threshold is above
     * 1000.
     *
    */
    ...
    packet_write(txpacket, PACKET_TYPE_DSP_CONTROL, -1);
        ...
        packet_add_field(txpacket, FIELD_TIME, -1);
            ...
            packet_add_param_int(txpacket, TIME_NUM_SAMPLES, 4096);
            /* To use power threshold triggers requires manual attenuation */
            packet_add_param_int(txpacket, TIME_MANUAL_ATTEN, 10);
            /* Do the capture at or above a power threshold */
            packet_add_param_int(txpacket, TIME_TRIG_MODE_POWER_THRESH, 1);
            /* The threshold level */
            packet_add_param_int(txpacket, TIME_TRIG_PT_THRESH, 800);
            /* The number of these triggers to capture */
            packet_add_param_int(txpacket, TIME_TRIG_NUMBER_CAPTURES, 10);
