/*
    Example showing how to request data using the BDC
...
int main(int argc, char* argv[])
{
    ...
    /* Create packet to request status information from node */
    /* We can use this to see how the BDC is connected up */
    T_PACKET* txpacket = packet_create("packet", 8192);
    packet_write(txpacket, PACKET_TYPE_NODE, -1);
    packet_add_field(txpacket, FIELD_STATUS_INFORMATION,0);
    ...
    // How is the BDC connected up?
    packet_read(rxpacket);
    int field_name, field_id;
    packet_get_next_field(rxpacket, &field_name, &field_id); /* Stat message only contians one field */
    char status_exp_1[255], status_exp_2[255];
    packet_get_param_string(rxpacket, STATUS_EXP_1, status_exp_1, 255);
    packet_get_param_string(rxpacket, STATUS_EXP_2, status_exp_2, 255);

    printf("Node Expansion Port 1: %s\n", status_exp_1);
    printf("Node Expansion Port 2: %s\n", status_exp_2);



    /* Create packet to request the bdc sweep */
    packet_write(txpacket, PACKET_TYPE_DSP_CONTROL, -1); /* Create DSP single command packet */
        /* Add field requesting sweep */
        packet_add_field(txpacket, FIELD_SWEEP, 0);
            /* Request full range */
            packet_add_param_int(txpacket, SWEEP_START_FREQ_MHZ, 10);
            packet_add_param_int(txpacket, SWEEP_STOP_FREQ_MHZ, 18000);

            /* Which BDC to use */
            packet_add_param_int(txpacket, SWEEP_INPUT, 1);
            packet_add_param_int(txpacket, ANY_BDC_EXPANSION, 1);
            packet_add_param_int(txpacket, ANY_INPUT_BDC, 2);
    packet_write_complete(txpacket);
